  ...
  //  printf("Product Matrix\n");
    ...
    if (argv[1]) {
       ...
       // printf( " input n = %d " , n);
        ...
        for (int i = 0; i < n * n; i++) A[i] = 3.0; // (float)rand() / 100;//;
        for (int i = 0; i < n * n; i++) B[i] = 4.0; // (float)rand() / 100;//;
        ...
        for (int s = 0; s < samples; s++) {
          ...
          // Start measuring time
            ...
            ProductMat_e(n, A, B, C);     //select ProductMat_a, ProductMat_b, ProductMat_c, ProductMat_d, ProductMat_f
    ...
    // Stop measuring time and calculate the elapsed time
            ...
            printf("%s\t%03d\t%05d\t%2.4f\t%2.4f \n", "ver(e)\t d\t x64\t", s, n, seconds, timeNormalized);
            fprintf_s(fp, "%s\t%05d\t%05d\t%2.4f\t %2.4f \n ","ver(e)\td\tx64\t", s, n, seconds, timeNormalized);    /* write data to the file */
        }
